...
export default {
  ...
  setup() {
    ...
    const show = ref(null);
    const seasonNumber = Number(route.params.seasonNumber);
  ...
  return store.isEpisodeFavorite(episodeId);
};

const toggleFavorite = (episode) => {
  store.toggleFavoriteEpisode(episode);
};

return { season, playEpisode, show, store, isFavorite, toggleFavorite };
...
<template>
  ...
  class="rounded-full p-3 bg-green-500 cursor-pointer hover:bg-pink-700 scale-100 hover:scale-150 transition-transform duration-200" 
  aria-hidden="true"
> 
  <svg 
    :class="{ 'text-gray-800': !isFavorite(episode.id), 'text-white': isFavorite(episode.id) }" 
    class="w-4 h-4" 
    aria-hidden="true" 
